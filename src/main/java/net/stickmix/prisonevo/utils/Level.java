...
public class Level {
    ...
    private final static LevelInfo[] LEVELS = new LevelInfo[]{
            ...
            new LevelInfo(100000000, 120000,
                    player -> player.getLocalData().containsKey("forest_keeper_killed"),
                    player -> {
                        if (player.getShards() >= 70) {
                            player.changeShards(-70);
                            return true;
                        }
                        return false;
                    },
                    Lists.newArrayList(
                            "&7Получение следующего уровня",
                            "&7требует убийства " + ForestKeeperScript.BOSS_NAME,
                            "&7в течение вашей игровой сессии (то есть",
                            "&7убийство сбрасывается при перезаходе",
                            "&7на игровой сервер PrisonEvo) или же",
                            "&7просто заплатите &a&l70 шардов&7."
                    )
            ), //30
            ...
            new LevelInfo(5000000000L, 160000,
                    player -> player.getLocalData().containsKey("blaze_king_killed"),
                    player -> {
                        if (player.getShards() >= 100) {
                    ...
                    Lists.newArrayList(
                            "&7Получение следующего уровня",
                            "&7требует убийства " + BlazeKingScript.BOSS_NAME,
                            "&7в течение вашей игровой сессии (то есть",
                            "&7убийство сбрасывается при перезаходе",
                            "&7на игровой сервер PrisonEvo) или же",
                            "&7просто заплатите &a&l100 шардов&7."
                    )
            ), //40
            ...
            new LevelInfo(30000000000L, 180000,
                    player -> player.getLocalData().containsKey("forest_keeper_heroic_killed"),
                    Lists.newArrayList(
                            "&7Получение следующего уровня",
                            "&7требует убийства " + ForestKeeperScript.BOSS_NAME,
                            "&7в &6&l&nгероическом &7режиме",
                            "&7в течение вашей игровой сессии (то есть",
                            "&7убийство сбрасывается при перезаходе",
                            "&7на игровой сервер PrisonEvo)."
                    )
            ), //45
            ...
            new LevelInfo(85000000000L, 200000,
                    player -> player.getLocalData().containsKey("aqua_lord_killed"),
                    player -> {
                        if (player.getShards() >= 125) {
                            player.changeShards(-125);
                            return true;
                        }
                        return false;
                    },
                    Lists.newArrayList(
                            "&7Получение следующего уровня",
                            "&7требует убийства " + AquaLordScript.BOSS_NAME,
                            "&7в течение вашей игровой сессии (то есть",
                            "&7убийство сбрасывается при перезаходе",
                            "&7на игровой сервер PrisonEvo) или же",
                            "&7просто заплатите &a&l125 шардов&7."
                    )
            ...
            new LevelInfo(250000000000L, 240000,
                    player -> player.getLocalData().containsKey("blaze_king_heroic_killed"),
                    Lists.newArrayList(
                            "&7Получение следующего уровня",
                            "&7требует убийства " + BlazeKingScript.BOSS_NAME,
                            "&7в &6&l&nгероическом &7режиме",
            ...
            new LevelInfo(500000000000L, 280000,
                    player -> player.getLocalData().containsKey("gladiator_killed"),
                    player -> {
                        if (player.getShards() >= 150) {
                            player.changeShards(-150);
                            return true;
                        }
                        return false;
                    },
                    Lists.newArrayList(
                            "&7Получение следующего уровня",
                            "&7требует убийства &6&lДиаваль, Последний Гладиатор",
                            "&7в течение вашей игровой сессии (то есть",
                            "&7убийство сбрасывается при перезаходе",
                            "&7на игровой сервер PrisonEvo) или же",
                            "&7просто заплатите &a&l150 шардов&7."
                    )
            ), //60
            ...
            new LevelInfo(8_500_000_000_000L, 400000,
                    player -> player.getLocalData().containsKey("aqua_lord_heroic_killed"),
                    Lists.newArrayList(
                            "&7Получение следующего уровня",
                            "&7требует убийства " + AquaLordScript.BOSS_NAME,
                            "&7в &6&l&nгероическом &7режиме",
            ...
            new LevelInfo(18_000_000_000_000L, 460000,
                    player -> player.getLocalData().containsKey("argus_killed"),
                    Lists.newArrayList(
                            "&7Получение следующего уровня",
                            "&7требует убийства &b&lАргус Порабощенный",
                            "&7в течение вашей игровой сессии (то есть",
                            "&7убийство сбрасывается при перезаходе",
                            "&7на игровой сервер PrisonEvo)."
                    )
            ...
            new LevelInfo(50_000_000_000_000L, 580000,
                    player -> player.getEvoItems().hasItem(150),
                    Lists.newArrayList(
                            "&7Получение следующего уровня",
                            "&7требует наличие меча &6&lИспепелитель&7."
                    )
            ), // 85
            ...
            new LevelInfo(70_000_000_000_000L, 680000
//                    player -> player.getLocalData().containsKey("balnazzar_killed"),
//                    Lists.newArrayList(
//                            "&7Получение следующего уровня",
//                            "&7требует убийства &4&lБальназзар, Повелитель Тьмы",
//                            "&7в течение вашей игровой сессии (то есть",
//                            "&7убийство сбрасывается при перезаходе",
//                            "&7на игровой сервер PrisonEvo)."
//                    )
            ...
            new LevelInfo(90_000_000_000_000L, 780000,
                    player -> player.getLocalData().containsKey("argus_heroic_killed"),
                    Lists.newArrayList(
                            "&7Получение следующего уровня",
                            "&7требует убийства &b&lАргус Порабощенный",
                            "&7в &6&l&nгероическом &7режиме",
            ...
            new LevelInfo(110_000_000_000_000L, 1_500_000,
                    player -> player.getLocalData().containsKey("balnazzar_heroic_killed"),
                    Lists.newArrayList(
                            "&7Получение следующего уровня",
                            "&7требует убийства &4&lБальназзар, Повелитель Тьмы",
                            "&7в &6&l&nгероическом &7режиме",
    ...
    public static long getNextLevelCost(GamePlayer gamePlayer) {
        ...
        if (current >= LEVELS.length)
            return 0;
        return Modifiers.getLongModifier().apply(LEVELS[current].getCost());
    }
    ...
    public static int getNextLevelBrokenBlocksRequired(GamePlayer gamePlayer) {
        ...
        if (current >= LEVELS.length)
            return 0;
        return Modifiers.getIntModifier().apply(LEVELS[current].getBlockBrokenRequired());
    }

    public static boolean checkPredicate(Player p, int level) {
        if (level > LEVELS.length || LEVELS[level - 1].getAdditional() == null)
            return true;
        return LEVELS[level - 1].getAdditional().test(PrisonEvo.getInstance().getAthenaManager().get(p));
    }
    ...
    public static double getPercentsToNextLevel(GamePlayer player) {
        ...
        val blocksPercents = player.getBlocksBroken() * 50D / blocksRequired;
